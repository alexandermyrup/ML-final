...
The full pipeline is bundled into a single Jupyter notebook (`scripts.ipynb`) that runs top-to-bottom.
...
```bash
# 1. Clone
git clone https://github.com/alexandermyrup/ML-final.git
cd ML-final

# 2. Install deps (Python 3.11+ recommended)
pip install -r requirements.txt

# 3. Get the data (one-time, ~300 MB)
...
Then **Run All**. End-to-end runtime is **~80 minutes** on a modern laptop, dominated by Script 03 (training, ~60 min) and Script 02 (Isolation Forest fit, ~5 min).
...
├── scripts.ipynb                   <- the entire pipeline (8 stages in one notebook)
└── data/
